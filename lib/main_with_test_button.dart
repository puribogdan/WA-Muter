...
class _MonitoringScreenState extends State<MonitoringScreen> {
  ...
  // NEW: Handle app lifecycle - check and ensure service is running
  Future<void> _handleAppLifecycle() async {
    _enhancedLog('📱 [APP LIFECYCLE] Handling app startup/resume...');

    try {
      await NotificationService.handleAppLifecycle();
      _enhancedLog('✅ [APP LIFECYCLE] Service lifecycle handled successfully');
    } catch (e) {
      _enhancedLog('❌ [APP LIFECYCLE] Error handling service lifecycle: $e',
          level: 'ERROR');
    }
  }
  ...
  void initState() {
    super.initState();
    _enhancedLog('🏠 MonitoringScreen initialized - APP STARTED/REOPENED');
    _enhancedLog('📱 [FLUTTER] App lifecycle: initState() called');
  ...
  void _updateServiceStatus() {
    ...
    if (serviceRunning != _isServiceRunning ||
      ...
      _enhancedLog(
          '🔄 Service status updated: Running=$serviceRunning, Uptime=$uptime');
    }
  }
  ...
  Future<void> _toggleForegroundService() async {
    if (_isServiceRunning) {
      ...
      try {
        ...
        _enhancedLog('❌ Failed to stope', level: foreground service: $ 'ERROR');
      ...
      try {
        ...
        _enhancedLog('❌ Failed to start foreground service: $e',
            level: 'ERROR');
  ...
  Widget build(BuildContext context) {
    return Consumer<ScheduleProvider>(
      builder: (context, scheduleProvider, child) {
        print(
            '[MonitoringScreen] Building with schedule: ${scheduleProvider.hasSchedule}');
        return Scaffold(
          appBar: AppBar(
  ...
  Widget _buildScheduleStatusCard(ScheduleProvider scheduleProvider) {
    print(
        '[MonitoringScreen] _buildScheduleStatusCard - hasSchedule: ${scheduleProvider.hasSchedule}');
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  scheduleProvider.hasSchedule
                      ? Icons.schedule
                      : Icons.schedule_outlined,
                  color: scheduleProvider.hasSchedule
                      ? Colors.orange
                      : Colors.grey,
                ),
                const SizedBox(width: 8),
              ...
              Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: scheduleProvider.isWithinSchedule()
                          ? Colors.green
                          : Colors.orange,
                    ),
                  ),
  ...
  // Quick Actions Card - WITH TEST BUTTON
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  Navigator.of(context).pushNamed('/schedule').then((_) {
                    print(
                        '[MonitoringScreen] Returning from schedule screen, refreshing...');
            ...
            const SizedBox(height: 8),
            // TEST BUTTON - Sync data to native location
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                onPressed: () async {
                  ...
                  try {
                    ...
                    print('🧪 [TEST] ✅ Sync completed! Native code should now find the data.');
                    
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Data synced to native! Test notification blocking now.'),
                          backgroundColor: Colors.green,
                        ),
                      );
                    }
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                      'Service Mode: ${NotificationService.isForegroundMode ? 'Foreground' : 'Background'}'),
                  Text(
                      'Service Start Time: ${_serviceStartTime != null ? _serviceStartTime!.toIso8601String() : 'N/A'}'),
                  Text(
                      'UI Monitoring: ${NotificationService.isListening ? 'Active' : 'Inactive'}'),
